...
export function calculateDailyStats(trades: Trade[]): DailyStats[] {
    ...
    return Object.entries(grouped)
        .map(([date, dayTrades]) => {
            ...
            const grossLosses = Math.abs(losers.reduce((sum, t) => sum + (t.pnl ?? 0), 0));
            const avgRR = dayTrades.reduce((sum, t) => sum + (t.riskReward ?? 0), 0) / (dayTrades.length || 1);
...
export function calculatePortfolioStats(trades: Trade[]): PortfolioStats {
    ...
    if (closedTrades.length === 0) {
        return {
            totalTrades: 0, totalPnl: 0, winRate: 0, averageWin: 0, averageLoss: 0,
            largestWin: 0, largestLoss: 0, profitFactor: 0, averageRR: 0,
            currentStreak: 0, bestStreak: 0, worstStreak: 0,
            averageHoldTime: '0m', sharpeRatio: 0, maxDrawdown: 0, maxDrawdownPercent: 0,
            dailyStats,
        };
    }
    ...
    const avgHoldTime = avgHoldMins >= 60 ? `${Math.floor(avgHoldMins / 60)}h ${avgHoldMins % 60}m` : `${avgHoldMins}m`;
    ...
    return {
        ...
        largestLoss: losers.length > 0 ? Math.min(...losers.map(t => t.pnl ?? 0)) : 0,
        profitFactor: grossLosses > 0 ? grossWins / grossLosses : grossWins > 0 ? Infinity : 0,
        averageRR: closedTrades.reduce((sum, t) => sum + (t.riskReward ?? 0), 0) / closedTrades.length,
        currentStreak,
        bestStreak,
        worstStreak,
