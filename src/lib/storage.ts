...
export async function saveTrade(userId: string, trade: Trade): Promise<void> {
    ...
    try {
        ...
        await setDoc(docRef, { ...trade, userId, updatedAt: new Date().toISOString() });
    } catch (error) {
...
export async function getTradeById(userId: string, id: string): Promise<Trade | undefined> {
...
export async function saveJournalEntry(userId: string, entry: JournalEntry): Promise<void> {
    ...
    try {
        ...
        await setDoc(docRef, { ...entry, userId, updatedAt: new Date().toISOString() });
    } catch (error) {
...
export async function savePlaybook(userId: string, playbook: Playbook): Promise<void> {
    if (!userId) return;
    try {
        const docRef = doc(db, 'playbooks', playbook.id);
        await setDoc(docRef, { ...playbook, userId, updatedAt: new Date().toISOString() });
    } catch (error) {
